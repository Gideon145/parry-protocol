...
const FALLBACK_RESPONSES: Record<string, string> = {
  ...
  "how does this work": `Parry runs a fully autonomous agent loop every 15 seconds:\n\n1. Fetches live ETH price via OnchainOS market skills\n2. Gets DEX directional signals via okx-dex-signal to inform hedge bias\n3. Computes delta exposure using exact Uniswap V3 math: Δ = L × (1/√S − 1/√p_b)\n4. Gets realized volatility to adjust hedge ratio (HIGH vol → hedge more)\n5. Runs periodic token security scans via okx-security before any swap\n6. If delta > threshold, swaps ETH→USDC via OKX DEX to neutralize exposure\n7. Records every action on X Layer Testnet — fully on-chain, fully verifiable\n8. Compounds accrued trading fees back into the LP position\n\nThe hedge API is gated by an x402 micropayment — pay only when protected.`,
